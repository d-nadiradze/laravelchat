...
socket.on('ConnectedUserArray', function (activeUserArray) {
    $.ajax({
        method: "GET",
        url: "/activeUsers",
        data: {
...
socket.on('activeUsers', (activeUserArray) => {
    $(".users").empty();
    activeUserArray.data.forEach((user) => {
        if (user.id == auth_user) {
            return;
        } else {
            $(".users").append(`
         <li id='${user.id}' userId='${user.id}'  class="active_user">
            <a class="hover:bg-gray-100 border-b border-gray-300 px-3 py-2 cursor-pointer flex items-center text-sm focus:outline-none focus:border-gray-300 transition duration-150 ease-in-out">
                ...
                <div class="w-full pb-2">
                    ...
                    <span class="last_message block ml-2 text-sm text-gray-600">${(user.last_message[0] ? user.last_message[0]['message'] : '')}</span>
                </div>
            </a>
         </li>
`)
    ...
    $("body").on('click', '.active_user', function () {
        ...
        $.ajax({
            ...
            success: function (message) {
                ...
                /** private chat header with user name */
                $('.chat-header').prepend(`
